...
def get_stage_name(days: int) -> Tuple[str, str, Optional[str]]:
    ...
    语义: 当前阶段 = 已达成的最新里程碑；两档之间为「已达成档名 + 期」
    """
    # 初生前夕（10 天前）
    if days < 10:
        return "初生前夕", "即将初识世界", "初识"

    # 百日之前（每 10 天一档：10-19 初识期 … 90-99 凝聚期）
    if days < 100:
        idx = (days - 10) // 10  # 0..8
        name = STAGES_100[idx][1]
        meaning = STAGES_100[idx][2]
        next_name = STAGES_100[idx + 1][1]
        return f"{name}期", meaning, next_name

    # 百日已过、周岁未到（100-364 天）
    if days < 365:
        return "百日期", "稳定成长，独立存在", "周岁"

    # 百日之后（按年：已达成档名 + 期，向下一里程碑进发）
    years = days // 365
    for i, (milestone_years, name, meaning) in enumerate(MILESTONES_YEARS):
        if years < milestone_years:
            prev = MILESTONES_YEARS[i - 1]
            return f"{prev[1]}期", prev[2], name
    last = MILESTONES_YEARS[-1]
    return last[1], last[2], None
